...
індекси, права). Відповідність моделей SQLAlchemy і схеми після міграцій перевіряє тест
(`tests/integration/test_migrations.py::test_models_match_migrated_schema`).
...
* **Свідомі похідні атрибути** (денормалізація заради знімка стану і швидкого `/explain`):
  `decision.kappa = f(agreement; κ_min, ν з run.config)`, `dq_score.score = f(completeness, validity, timeliness, continuity; ваги з config/dq_weights.yaml)`,
  ...
  Параметри функцій (κ_min, ν, ваги якості) можуть змінитися між прогонами, тому збережене значення — факт моменту обчислення,
...
Фізичну модель розкладено на ревізії Alembic:
...
у 1.8 раза більший за PK. Індекс залишено як приклад того, як план запиту перевіряє проєктне рішення.
