...
Веб-панель (фаза 9): `make ui` підіймає Vite на `http://localhost:5173` і проксіює `/api` на API (`FUZZHELM_API`, типово
`http://127.0.0.1:8000`), тож CORS не потрібен. Три екрани — **Виведення** (`/explain/{id}`, кульмінація демо), **Онлайн**
(`/live`) і **Бектест** (`/backtest`); інтерфейс українською, перемикач `UK/EN` у шапці. Щоб «Онлайн» наповнився, паралельно
має працювати воркер реплею.

У контейнерах: `docker compose up -d --build` піднімає **чотири сервіси** — `db`, `api` (`127.0.0.1:8000`), `worker`
(реплей профілю `replay`) і `ui` (`5173`). Докладно — [`docs/manuals/deployment.md`](docs/manuals/deployment.md).
...
**Зроблено:** Python-бекенд (фази 0–8 брифінгу), обчислювальний експеримент фази 7 з результатами, **веб-панель фази 9**
(три екрани Vue 3 + Pinia + ECharts, i18n uk/en, 14 екранограм для звіту), добивання фази 10 у частині коду (покриття, mypy,
CI, MLP у контурі) і документації (ТЗ, реєстр ризиків, розходження, журнал, таблиці звіту).
