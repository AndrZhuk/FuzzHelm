...
class DrawdownSpeedBound:
    ...
    1) збиток позиції за бар обмежений її ризик-бюджетом: |ΔE| ≤ κ·ρ_base·E_t·(1 + κ_slip) —
    ...
    3) κ — це κ_mode НА МОМЕНТ ВХОДУ відкритої позиції (κ_entry), а не поточного стану. Рушій фіксує розмір
       на вході і не зменшує відкриту позицію, коли автомат знижує κ_mode (ENG-04): після переходу
       NORMAL → WARNING/COOLDOWN позиція, відкрита з κ = 1, ризикує бюджетом ρ·|u|·E_entry, а не
       κ_mode(поточний)·ρ·|u|·E. Тож v_max(COOLDOWN) = κ_mode(COOLDOWN)·ρ·(1+κ_slip) діє лише для позицій,
       відкритих у COOLDOWN (політика scaled_entries); поки жива позиція, відкрита до COOLDOWN, межа — з її
       κ_entry; за reduce_only після її закриття v = 0 (пласка книга, нової експозиції немає). Монотонність
       κ_mode за DD (гарантує автомат) стосується лише НОВИХ входів;
    4) E_t у межі — капітал на вході (бюджет позиції рахується від E_entry); при E_t < E_entry частка збитку
       від поточного капіталу більша в E_entry/E_t разів;
    5) DD рахується від піку, що містить нереалізований прибуток за закриттями барів: якщо позиція спершу
       виросла до майже тейку (m·Δ_stop, m = take_profit.multiple_of_stop, ENG-01), а потім за один бар
       дійшла до стопа, приріст DD за бар — до (1 + m + κ_slip)·κ_entry·ρ, а не (1 + κ_slip)·κ_entry·ρ;
       per_bar_loss_bound — буквальна формула §5.12, яка цей відкат не враховує;
    6) витрати виконання (комісія taker на вході й на виході, sqrt-імпакт, фандинг) у межу не входять —
       κ_slip моделює лише проковзування стопа. Номінал позиції сайзер обмежує κ·L_max·E (q_lev), тож комісія
       кожного боку — до f_taker·κ·L_max·E_entry: за f_taker = 0.0004, L_max = 3 це до 0.12 %·κ·E на бік,
       до 0.24 %·κ·E за коло (вхід і стоп в одному барі) — порівнянно з самим бюджетом κ·ρ = 0.5 %·κ.
    Тоді за 1)–4), без відкату 5) і без витрат 6): DD_{t+1} − DD_t = (E_t − E_{t+1})/peak ≤ |ΔE|/E_t ≤ v_max,
    і n_min ≥ (DD_max − DD₀)/v_max. Порушення будь-якого припущення (геп через стоп, ліквідація, кілька
    корельованих позицій, позиція з вищим κ_entry, відкат нереалізованого прибутку, комісії) робить межу
    недійсною — тому це не доведення стійкості.
    """
...
def per_bar_loss_bound(kappa_mode: Decimal, rho_base: Decimal, kappa_slip: Decimal) -> Decimal:
    """v_max = κ·ρ_base·(1 + κ_slip) — найбільша частка капіталу, втрачена за бар (див. припущення
    DrawdownSpeedBound; κ — κ_mode на вході відкритої позиції, а не поточний стан автомата, ENG-04; без
    відкату нереалізованого прибутку і без комісій/імпакту/фандингу)."""
